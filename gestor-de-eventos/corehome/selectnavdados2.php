...
include('dados_conexao.php');
	...
	try {
		...
		$consulta = $conecta->prepare("SELECT tb1_evento.*, (SELECT count(*) from tb1_evento) totaleventos FROM tb1_evento WHERE tb1_codfuncionario = '$_SESSION[codfuncionario]' LIMIT 1");
	...
	if ( count($resultadoDaConsulta) ) {
		foreach($resultadoDaConsulta as $registro) {
			...
			$StringJson .= '{"totaleventos":"' . $registro['totaleventos']  . '",';
